...
function get_tracers(RMAX, rv::Vector{Float64}, xv::Vector{Float64}, yv::Vector{Float64}, zv::Vector{Float64}; 
    ...
    for v in 1:nvoids
        ...
        trac_lists[v] = hcat(tcat[mask,:], sq_distance[mask])
        trac_lists[v] = trac_lists[v][sortperm(trac_lists[v][:,end]),:]
    end
...
function main()
    ...
    Rv_min, Rv_max, z_min, z_max, rho1_min, rho1_max, rho2_min, rho2_max, flag = 10., 12., 0.2, 0.25, -1., -0.9, -1., 100., 2
